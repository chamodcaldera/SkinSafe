...
def connection():
    ...
    mycursor.execute("CREATE TABLE IF NOT EXISTS Chanelling (channelId INT NOT NULL AUTO_INCREMENT, id INT NOT NULL,channel_date VARCHAR (10) NOT NULL,channel_time  VARCHAR (10)  NOT NULL , doctorId INT NOT NULL, status VARCHAR(10),message VARCHAR(100), PRIMARY KEY (channelId), FOREIGN KEY (id) REFERENCES User(id), FOREIGN KEY (docterId) REFERENCES doctor(docId))")  # Execute SQL Query to create a table into your database
    ...
    mycursor.execute("CREATE TABLE IF NOT EXISTS DoctorTimeSlots (timeSlotId INT NOT NULL AUTO_INCREMENT,doctorId INT NOT NULL  ,day VARCHAR (20) NOT NULL ,timeStart VARCHAR(10) NOT NULL, timeEnd VARCHAR(10) NOT NULL, PRIMARY KEY (timeSlotId),FOREIGN KEY (doctorId) REFERENCES doctor(docId))")  # Execute SQL Query to create a table into your database
